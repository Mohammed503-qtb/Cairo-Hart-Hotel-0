...
---
Task ID: 24 (F7 + F8 + P — التنفيذ الكامل)
Agent: Main Agent (Z.ai Code)
Task: تنفيذ وإكمال F7 (التجربة الداخلية Dogfooding) + F8 (تقديم المتاجر) + المرحلة P كاملة (P1–P7) بأمر المالك «قم الان بإكمال وتنفيذ F7 and F8 and P»

Work Log:
- قراءة الذيل الكامل لسجل العمل + MASTER_PLAN §6.8/6.9/6.10 (بوابة المتجر) + §8 (P1–P7 + بوابة 8.8) + حالة البيئة (خادم 3000 حي · realtime 3002 حي · git نظيف على 7290a95)
- [24-a subagent] docs/DOGFOOD_PLAN.md (بروتوكول 14 يومًا + تصنيف SEV-1/2/3 + AD-09 الحرفي) + docs/DOGFOOD_FINDINGS.md (قائمة إصلاح مُدارة بقالب DF-NNN)
- [24-b subagent] docs/PRIVACY_POLICY.md v1.0 (SSOT — 9 أقسام) + docs/STORE_SUBMISSION.md (بوابة §6.10 + نصوص Play + Data Safety + 8 لقطات + AAB قرار)
- [24-c subagent] deploy/ كامل (Caddyfile.prod بآلية XTransformPort=3002 حرفيًا + systemd للخدمتين + deploy.sh بrollback + env.production.example + README) + docs/BACKUP_POLICY + MONITORING + RUNBOOK (6 بطاقات) + SECURITY_CHECKLIST + PAYMENT_GATEWAY_DECISION (مفتوحة عمدًا) + LAUNCH_PLAN + RESTORE_DRILL — bash -n وcaddy validate مرّا
- [24-d كود] scripts/sqlite-tool.ts (bun:sqlite — VACUUM INTO متناسقة + quick_check + restore بأمان تلقائي) + backup.sh/restore.sh فوقها (احتفاظ 7/4/6 + rclone اختياري) + purge-demo.ts (AD-10: مسح كامل تشغيلي + جرد صفر تام + كود إقلاع أدمن خام مرة واحدة + تدقيق DATA_PURGED) + smoke.ts (رحلة ذهبية 25 فحصًا حيًا عبر HTTP)
- [24-e كود] GET /api/health (يتحقق من القاعدة فعليًا SELECT 1 · 503 عند التعطل · version من package.json) + scripts/health-check.sh (health/الرئيسية/3002/3004/القاعدة/القرص + تنبيه WEBHOOK عند تغيّر الحالة فقط عبر state file) + تفعيل فحص الصحة في Caddyfile.prod + مداخل package.json (smoke/purge-demo/backup/restore/health-check) + تحديث الوثائق الثلاث لتطابق التنفيذ
- [24-f كود] الويب: privacy-dialog.tsx (مرآة موجزة للسياسة) مربوط في بطاقة السياسات + عمود تذييل «السياسات» · الجوال: screens/shared/privacy_screen.dart (السياسة + الإصدار kAppVersion) + زر درع في GuestShell + 4 اختبارات جديدة
- [24-g Day-0 منفَّذ فعليًا]: ① purge --confirm محى 106 صفوف (جرد صفر تام) وأصدر كود إقلاع أدمن خامًا (لا يُدوَّن الكود الخام في أي ملف مستودع — درس الحادثة أدناه) ② smoke بالكود الحقيقي: **25/25 أخضر** (HTL-2026-000001 · ST-2026-000001 · REQ-1000 — المراجع عادت من الصفر صح) ③ backup: نسخة 278KB بquick_check ok ④ **تدريب استعادة كامل حقيقي**: إيقاف الخادم → rm القاعدة → restore.sh → الأعداد مطابقة تمامًا لما قبل الكارثة (access_codes 3 · audit_logs 17 · guests 1 · messages 2 · notifications 10 · payments 1 · reservations 1) → الخادم عاد health 200 (67ms) → موثق بالصف الأول في RESTORE_DRILL §4 (≈ دقيقتان ≪ ساعة البوابة) ⑤ health-check.sh: HEALTHY حيًا
- [24-h DEP-01] ترقية next 16.1.3 → **16.3.4** (داخل caret · فوق 16.2.11 الأمنية المطلوبة) + eslint-config-next 16.3.4 → إعادة تشغيل الخادم (next-server v16.3.4) → **lint نظيف + 165 اختبارًا أخضر (124+7+34) exit 0**
- [التحقق بالمتصفح — agent-browser] الرئيسية كاملة بلا أخطاء console · حوار الخصوصية يفتح بكل أقسامه · **حجز كامل عبر الواجهة**: تواريخ → بحث → اختيار غرفة → بيانات الضيف → إقرار سياسة الإلغاء → دفع بالفندق → **HTL-2026-000002** · «إدارة حجزك»: بحث بمرجع+هاتف → مؤكد + الضيف + سياسة الإلغاء · 375px صفر فيض أفقي · 1440px نظيف · dev.log نظيف
- [الإطلاق] git commit 2cca2a0 → push → Web CI + Mobile CI انطلقا (المراقبة جارية حتى الخضار)

Stage Summary:
- **F7**: العتاد كامل ومنفَّذ (بروتوكول + مسح AD-10 + smoke 25/25 + قائمة إصلاح) — التقويم الحي (14 يومًا بلا SEV-1) مسؤولية المالك بصراحة موثقة في DOGFOOD_PLAN §8
- **F8**: الحزمة كاملة (سياسة SSOT ويب+جوال + نصوص المتجر + Data Safety) — التقديم نفسه بيد المالك بعد شهادة أسبوعي F7 (بوابة §6.10 موثقة الحالة)
- **P**: P1 عتاد deploy/ كامل · P2 مقفولة فعليًا بالتدريب الحقيقي الموثق (بوابة 8.8: بند الاستعادة فُتح) · P3 حية (health + مراقبة) · P4 RUNBOOK · P5 قرار مفتوح عمدًا · P6 قائمة + DEP-01 مُصلح فعليًا (next 16.3.4) · P7 خطة إطلاع وتراجع
- المتبقي بيد المالك: تشغيل VPS من deploy/README · DOMAIN + قناة التنبيه + وجهة rclone · شهادة F7 التقويمية · بيانات المتجر التجارية · قرار AAB وP5

---
Task ID: 25 (التحقق النهائي والإطلاق)
Agent: Main Agent (Z.ai Code)
Task: CI على GitHub حتى الخضار + تحديث MASTER_PLAN بحالات F7/F8/P الحقيقية + الإغلاق

Work Log:
- Push الأول (2cca2a0): Web CI نجح من أول مرة (كل تغييرات الويب خضراء) · Mobile CI فشل في 3 اختبارات من شاشة الخصوصية الجديدة: ListView الكسول لا يبني الأقسام خارج نافذة الاختبار 800×600 (find.text=0 للأقسام الأخيرة + StateError في ensureVisible) — الدرس: الشاشات الثابتة النصية في المشروع تُبنى بـ SingleChildScrollView (نمط update_required_screen) لا ListView
- الإصلاح (841847a): تحويل PrivacyScreen إلى SingleChildScrollView+Column (توازن الأقواس متحقق بالفحص الآلي) → دفع
- **Mobile CI على 841847a: نجح كاملًا — 173 اختبار Flutter (169 سابقة + 4 جديدة للخصوصية) + Android build smoke ✓** · Web CI على 2cca2a0: نجح (تغيير 841847a جوال-فقط)
- تحديث MASTER_PLAN بالحالات الحقيقية: §6.8 F7 🟡 (العتاد + Day-0 منفَّذ · التقويم بيد المالك) · §6.9 F8 🟡 (الحزمة كاملة · التقديم بعد بوابة §6.10) · §8 P 🟢 (الحزمة كاملة · P2 مقفولة فعليًا بالتدريب الموثق · التنفيذ على الخادم بيد المالك)
- فحص ختامي: /api/health 200 · الرئيسية 200 · dev.log نظيف

Stage Summary:
- **CI خضراء بالكامل على GitHub بعد إصلاح واحد** — المسار الحي للأدلة يعمل كما في كل جولة سابقة
- MASTER_PLAN الآن يطابق الواقع: ما نُفِّذ فعليًا موثَّق بالتفصيل، وما بقي بيد المالك مصنف صراحة (تقويم F7 · تقديم F8 · VPS/DOMAIN/التنبيه/قرار P5)
- إجمالي المخرجات للجلسة: 13 وثيقة + 6 سكربتات + مسار health + خصوصية ويب وجوال + ترقية next الأمنية + تدريب استعادة حقيقي + Day-0 كامل — كلها في دفعتين على GitHub

---
Task ID: 25-ب (إصلاح حادثة الالتزام الزائد + إبطال الاعتماد المسرب)
Agent: Main Agent (Z.ai Code)
Task: فحص التزام الإغلاق f4045c3 كشف إدخالاً خاطئًا (1052 ملف skills/ + نسخ القاعدة الاحتياطية + كود إقلاع خام في worklog) — الجذر: غياب سطر جديد نهاية .gitignore جعل echo يلصق «backups/» بنمط «/skills» ففسدا معًا → إصلاح جذري كامل

Work Log:
- الجذر التشغيلي: .gitignore كان ينتهي بـ«/skills» بلا سطر جديد — `echo "backups/" >>` دمج السطرين في نمط ميت واحد
- git reset HEAD~1 (فك الالتزام مع إبقاء شجرة العمل) + إصلاح .gitignore النهائي: «/skills» سليمًا سطرًا مستقلًا + backups/ + rollbacks/ — تحقق بـgit check-ignore للثلاثة ✓
- **إبطال الاعتماد المسرب**: الكود الخام كان في worklog الملتزم — أُعيد تشغيل purge --confirm فمات الكود القديم وجلساته كلها (محيت 50 صفًا) وصدر كود إقلاع بديل جديد (خام — يُعرض للمالك في المحادثة فقط، لا يُخزَّن في أي ملف)
- إعادة التحقق الحي بعد المسح الجديد: smoke 25/25 أخضر + health-check: HEALTHY (health/home/realtime/emit/db)
- حجب الكود من worklog (الاستبدال بملاحظة الدرس) + هذا القسم نفسه يوثق الحادثة بلا الكود
- الالتزام البديل: MASTER_PLAN + worklog + .gitignore فقط — **force-push** يزيل f4045c3 من تاريخ GitHub كاملًا (ملفات skills + نسخ القاعدة + الكود الخام تختفي من المستودع البعيد — القاعدة الخاصة محلية والملفات على القرص كما هي بلا تتبع)
...
Task ID: 27
Agent: Main Agent (Z.ai Code)
Task: أمر المالك — مراجعة لوحة تحكم الأدمن ولوحة تحكم الاستقبال في تطبيق الهاتف: فحص واختبار فعلي وتكامل مع قواعد البيانات والـ Backend وكل عملية وشاشة + ثيم متطابق لا يخرب الألوان + نصوص متطابقة مع كل مقاسات الهواتف — ب دليل فعلي لا انطباع أولي

Work Log:
- [البيئة] تثبيت Flutter 3.29.2 محليًا (نفس نسخة CI) — الخادم 3000 حي + تشغيل realtime 3002 (كان متوقفًا) + emit 3004
- [التدقيق الساكن] وكيل قراءة صنّف 144 موضع AppColors معنوي في 32 ملفًا: ~77 موضعًا معطوبًا في 19 ملفًا جذورها 4 مكونات مشتركة — التطبيق يتبع ثيم النظام فالوضع الداكن سيناريو مستخدم حقيقي
- [الدليل الأحمر] mobile/test/ui/theme_contrast_test.dart: قياس WCAG حقيقي للرقائق المُصيَّرة في الوضعين (تركيب الشفاف 15% فوق بطاقة داكنة كالرسم الفعلي) — 10 إخفاقات موثقة بأرقام: الوضع الداكن 1.00:1 (نص غير مرئي في 4 مصانع رقائق × 25 استخدامًا) + CLEANING فاتحًا 1.86:1 + KpiCard coral 2.41:1 + MoneyText 2.75:1
- [الإصلاح الجذري 27-الرئيسي] theme.dart: توكنات داكنة مرآة .dark للموقع (#4ADE80/#FBBF24/#F87171/#8FB3E3) + semantic/successOf/warningOf/dangerOf/infoOf/resolveTone/darkTint · إصلاح كل مصانع StatusChip (7) بمنوال الويب: فاتح (نغمة داكنة/حاوية صلبة) داكن (نغمة فاتحة/صبغة 15%) · KpiCard · MoneyText · RawCodeBox · AdminAlertBox (نغمات) · شبكة الغرف والدليل · معالجي الوصول/الخروج
- [27-a وكيل] مسح 9 ملفات استقبال (~59 موضعًا + هياكل) · [27-b وكيل] 6 ملفات أدمن (13+ مواضع) — كلها بنمط *Of(context) والخلفيات مشتقة من نفس النغمة المحلولة
- [الدليل الأخضر] theme_contrast: 28/28 ✅ (كان 10 إخفاقات)
- [المقاسات — الدليل] responsive_dark_test.dart بخط Cairo الحقيقي (Ahem يقيس العربية 2x فيولد فيضًا وهميًا — حُمّل Regular/SemiBold/Bold + monospace): 17 شاشة × 4 مقاسات (320/360/412/768) × تكبير 1.3 + الأصداف ضيقًا/واسعًا + التصيير الداكن لكل شاشة + حوار البحث
- [أعطال مقاسات حقيقية اكتُشفت وأُصلحت]: ① KpiCard يقص عموديًا في شبكة أدمن childAspectRatio:1.5 (فيض 20-40px × 28 موضعًا) → Wrap بعرض ثابت وارتفاع ذاتي (كشبكة الاستقبال وgrid الويب) ② صفوف أزرار Row بدل flex-wrap الويب في الوصولين (170px!)/المغادرون (68px)/لوحة الاستقبال (58px)/حجوزات الأدمن → Wrap(end/spaceBetween) ③ عارضا الصفحات pager (123px/68px) → Wrap مزدوجة ④ notifyListeners أثناء بناء السلف (markNeedsBuild-during-build) في 17 شاشة عبر initState→_refresh() → addPostFrameCallback بجميع الحرس — كله كان مخفيًا لأن الاختبارات القديمة لا تضخ الأصداف
- [التكامل الحي 67/67] scripts/mobile-contract-test.ts (bun run mobile-contract): أكواد حقيقية (make-codes.ts) → دخول أدمن: كل A-01..A-34 قراءات بأشكال نماذج الجوال حرفيًا + دورات CRUD كاملة (نوع غرفة/غرفة/معدل/فئة/خدمة/موظف+كود مولّد يُبطَل) مع تنظيف → حجز عام → دخول استقبال: كل R-01..R-23 + المسار الحي كامل (وصول بكود ضيف خام → تفصيل إقامة → حالة طلب → دفعة → بند SERVICE → رسائل → تمديد كضيف والبت رفض → خروج بتسوية صفر → موت كود الضيف I11) → renew §1.2.1 للدورين → 401/403 الحرس → حارس الانتقالات (AVAILABLE→CLEANING مرفوض 400 عربي) → حد المعدل 5/دقيقة (اشتغل فعليًا) → socket.io اتصال + join reception + استقبال بث من 3004 → خروج + موت الجلسة → تنظيف الغرف DIRTY إلى AVAILABLE
- [اكتشاف منهجي مهم] اختبارات الجوال القديمة كانت تحاكي أشكال حمولات خاطئة أحيانًا (nameAr بدل name، مسطحة بدل متشعبة) — نماذج الجوال نفسها متطابقة 100% مع الـ Backend (قُوبنت حقلًا-حقلًا)؛ الحمولات في الاختبارات صُححت على الأشكال الحقيقية
- [الختام] flutter analyze صفر أخطاء · flutter test 295/295 ✅ · bun run lint نظيف · dev.log نظيف · الويب 200 + health ok · أُبطلت أكواد الفحص بعد الانتهاء (تُنشأ بmake-codes عند الحاجة)

Stage Summary:
- الثيم: الوضع الداكن كان مكسورًا فعليًا (نص 1.00:1 غير مرئي في 25 موضع رقائق + ~77 موضعًا إجمالًا) — أُصلح بمرآة توكنات .dark للموقع وأصبح متطابقًا مع الويب في الوضعين (اختبار WCAG 28/28 دائم)
- المقاسات: 5 أنماط فيض حقيقية أصلحت (KPI grid، 4 صفوف أزرار/pager، markNeedsBuild أثناء البناء) — اختبار دائم 17 شاشة × 4 مقاسات × 1.3 + داكن + أصداف
- التكامل: عقد الجوال-الخادم متطابق 100% (أثبته 67 فحصًا حيًا شاملًا كل عملية A وR والمسار الذهبي والحراس وrealtime) — قابل لإعادة التشغيل: bun run make-codes ثم bun run mobile-contract
- البنية الجديدة: 3 ملفات اختبار دائمة (تباين/مقاسات+داكن) + سكربتا عقد وإنشاء أكواد + إدخال package.json

---
Task ID: 28
Agent: Main Agent (Z.ai Code)
Task: أمر المالك — تجهيز المشروع للنشر الإنتاجي التجاري الفعلي لمستخدمين حقيقيين + رفع التحديثات إلى المستودع

Work Log:
- [التقييم] تقرير التدقيق خُتم بـ 3 MAJOR موقوفة: REQ-01 (تعديل الحجز غائب بلا قرار) · MOB-01 (APK متقادم بُني قبل F4/F5/F6) · DEP-01 (كان أُصلح: next 16.3.4) + MINOR/INFO قابلة للإغلاق الرخيص (DOC-01/MOB-04/DEP-03/LIVE-01) — كلها عولجت في هذه الموجة
- [REQ-01 · قرار موثق ونفاذ] المسار C «تعديل الحجز المؤهل» منفذًا كاملًا: POST /api/public/edit (PUB-08) — الأهلية CONFIRMED + قبل (الوصول−24س) · تحقق مرجع+هاتف بلا كشف وجود · توفر داخل المعاملة مع استثناء الحجز نفسه (availableRoomCount opts.excludeReservationId — لا oversell ولا self-lockout) · إعادة تسعير خادمية computeQuote + لقطة buildSnapshot جديدة · paidCents تُنقل كما هي (بلا حركات مالية وهمية — الفرق يُسوّى عند الفندق) + paymentStatus بمعادلة payments نفسها · تدقيق RESERVATION_MODIFIED (قبل/بعد كاملة) + إشعار استقبال + بث reservation:modified (معالجات توست في reception-app وadmin-app)
- [REQ-01 · الواجهة] edit-booking-form.tsx: نموذج كامل (تواريخ/نوع/ضيوف/غرف/طلبات خاصة) بمعاينة حية debounce من availability بامتداد الاستثناء الذاتي (reference+phone اختياريان — فشل التحقق يُتجاهل بصمت) · ملخص قبل/بعد بفروق الأسعار والفرق لصالح الضيف · زر «تعديل الحجز» في إدارة الحجز للأهلية فقط + نص النافذة في بطاقة سياسة الإلغاء · شكل الاستجابة = شكل lookup فيُحدَّث العرض سلسًا
- [REQ-01 · الاختبارات] 8 اختبارات تكامل c1–c8 في flows.test.ts: تمديد ليالٍ (تسعير+لقطة+تدقيق+إشعار) · تغيير نوع (تحرير/حجز المخزون) · معاينة الاستثناء الذاتي (صحيح/خاطئ الهاتف) · تعديل على مخزون ممتلئ بلا self-lockout + الهروب · 409 فوق المخزون بلا كتابة · مدفوع CARD يُنقل (paymentStatus PAID بلا صفوف دفع جديدة) · حراس النافذة/الهاتف/بلا تغييرات · ملغى/مقيم غير قابلان — كلها فوق expectedQuote المستقل بالسنت
- [MOB-01] رفع mobile/pubspec.yaml إلى 1.1.0+2 + تحديث نصوص إصدار mobile-release (الأوضاع الثلاثة + الحارس) — دفع وسم v1.1.0 يشغّل بناء APK موقّع من أسرار المستودع يحمل F1+F4+F5+F6+إصلاحات Task 27
- [النظافة] حذف ملف الخردة --fullpage من التتبع + .gitignore · MOB-04: حذف tailwind.config.ts الأجوف + اعتمادية tailwindcss-animate الميتة (bun.lock محدث، الموقع 200) · DEP-03: أرشفة mobile/pubspec.lock + تثبيت bun 1.3.14 في web-ci (بدل latest) · LIVE-01: توست ظاهر عند التصحيح التلقائي لتاريخ المغادرة
- [التوثيق] CONTRACTS v1.2 (بطاقة PUB-08 كاملة بكل الرسائل الحرفية + امتداد PUB-03 + سجل v1.2) · MASTER_PLAN v2.6 (تصويب العنوان v2.4→v2.6 + قرار المالك REQ-01 موثق في قسم P + سجل 2.6) · AUDIT_REPORT §0 إغلاق ما بعد التدقيق (جدول مصيرات الـ 3 MAJOR + MINOR/INFO) · README (173 اختبارًا/22 نموذجًا/قسم النشر الإنتاجي 8) · LAUNCH_PLAN (purge-demo موجود بالمستودع)
- [التحقق] lint صفر · اختبارات الويب 173/173 (124+7+42) · flutter analyze صفر أخطاء (42 info معروفة) · flutter test 294/294 · العقد الحي 67/67 (realtime + renew + 401 + I11) · smoke 25/25 · متصفح حي end-to-end: بحث حجز حقيقي → زر تعديل → نموذج بمعاينة حية ($202.40→$294.40) → حفظ → التفاصيل محدثة (3 ليالٍ) + القاعدة (29440=3 ليالٍ بزيادة نهاية الأسبوع) + تدقيق RESERVATION_MODIFIED + إشعار — 375px صفر فيض أفقي · صفر أخطاء console · صور EV-PREQ01 جهاز/محمول
- [الانضباط] الحجز التجريبي الحي أُزيل بعد التحقق (استعادة الحالة) · أكواد فحص العقد أُبطلت (REVOKED + جلساتها)

Stage Summary:
- المشروع جاهز للنشر التجاري: كل نتائج التدقيق MAJOR مغلقة بتنفيذ مثبت (REQ-01 نفذ المسار C كاملًا · MOB-01 إصدار v1.1.0 موسوم · DEP-01 كان مُصلحًا) + MINOR/INFO الرخيصة مغلقة + عتاد P1-P4 جاهز في deploy/ بلا تغيير
- الميزة الجديدة للضيوف الحقيقيين: تعديل حجزهم من الموقع (مواعيد/نوع غرفة/ضيوف/طلبات) بنافذة 24 ساعة — بمعاينة سعر حية وتسعير خادم حتمي ولقطة جديدة وتدقيق كامل — 8 اختبارات تكامل + تحقق متصفح حي
- البطارية الختامية: 173 ويب + 294 جوال + 67 عقد حي + 25 smoke — كلها خضراء · dev.log نظيف · الصحة ok
- المتبقي على المالك (خارج المستودع): تشغيل VPS من deploy/README + النطاق + قناة التنبيه + شهادة F7 الأسبوعين (LAUNCH_PLAN §5) — P5 (مزود الدفع) قرار عمل لا يمنع الإطلاق §8.8
